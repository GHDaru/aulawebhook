...
export async function authenticateUser(identifier, password) {
  ...
  const user = users[0]
  if (user.senha_hash !== hashPassword(password || '')) return null
...
export async function createAlunosFromCsv({ csvText, defaultPassword }) {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      // ignora duplicados em importação em massa
    }
  }
...
export async function upsertProgresso({ alunoId, disciplinaId, concluido, total }) {
  ...
  const percentual = parsedTotal > 0 ? Number(((parsedConcluido / parsedTotal) * 100).toFixed(2)) : 0
  ...
  if (existing.length) {
    await sql`
      ...
      SET concluido = ${parsedConcluido}, total = ${parsedTotal}, percentual = ${percentual}, updated_at = NOW()
      WHERE id = ${existing[0].id}
    `
    return { id: existing[0].id, alunoId, disciplinaId, concluido: parsedConcluido, total: parsedTotal, percentual }
  }

  const id = generateId('progresso')
  await sql`
    INSERT INTO progresso_aluno (id, aluno_id, disciplina_id, concluido, total, percentual)
    VALUES (${id}, ${alunoId}, ${disciplinaId}, ${parsedConcluido}, ${parsedTotal}, ${percentual})
  `

  return { id, alunoId, disciplinaId, concluido: parsedConcluido, total: parsedTotal, percentual }
}
